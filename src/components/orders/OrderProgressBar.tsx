...
import { AlertCircle, Banknote, CheckCircle2, Clock, CreditCard, Package, Truck, XCircle } from 'lucide-react';
      ...
      <div className="bg-red-50 border border-red-200 rounded-lg p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-red-700 mt-1">{motivoCancelacion || 'Este pedido ha sido cancelado.'}</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="bg-white rounded-lg shadow-sm border p-6">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">{steps[currentStepIndex]?.description || 'Procesando...'}</p>
        </div>

        {/* Indicador de estado de pago */}
        {estadoPago && (
          <div className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium ${paymentClass}`}>
        ...
        <div className="relative flex justify-between">
          {steps.map((step, index) => {
            ...
            const showPaymentWarning =
              step.requiresPayment && isCurrent && isPaymentPending && step.key === 'Pendiente';
            ...
            return (
              <div key={step.key} className="flex flex-col items-center flex-1">
                ...
                >
                  {showPaymentWarning ? <Banknote className="h-5 w-5" /> : <Icon className="h-5 w-5" />}
                </div>

                {/* Etiqueta del paso */}
                <span className={`mt-2 text-xs font-medium text-center max-w-[80px] ${labelClass}`}>{step.label}</span>

                {/* Indicador de esperando pago */}
                {showPaymentWarning && (
                  <span className="mt-1 text-xs text-yellow-600 font-medium text-center">Esperando pago</span>
                )}
              </div>
            );
          })}
      ...
      {isPaymentFailed && estado === 'Pendiente' && (
        <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-lg flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-red-900 font-medium">Pago fallido</p>
            <p className="text-red-700 text-sm mt-1">
              El pago no se pudo procesar. Por favor, intenta nuevamente con otro método de pago.
            </p>
            {metodoPago && <p className="text-red-600 text-sm mt-2">Método: {metodoPago}</p>}
          </div>
        </div>
      )}
      ...
      {isPaymentPending && estado === 'Pendiente' && !isPaymentFailed && (
        <div className="mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-yellow-900 font-medium">Pago pendiente</p>
        ...
        <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          {numeroSeguimiento && (
            <p className="text-blue-700 text-sm mt-1">
              ...
              <span className="font-mono font-medium bg-blue-100 px-2 py-1 rounded">{numeroSeguimiento}</span>
            </p>
          )}
        </div>
      )}
      ...
      {estado === 'Entregado' && (
        <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg flex items-center gap-3">
          ...
          <div>
            <p className="text-green-900 font-medium">¡Pedido entregado!</p>
            <p className="text-green-700 text-sm">Gracias por tu compra. Esperamos que disfrutes tu producto.</p>
          </div>
        </div>
      )}
    </div>
