...
export function InvoiceNotAvailableModal({
...
}: Readonly<InvoiceNotAvailableModalProps>) {
  useEffect(() => {
    if (isOpen) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div
        className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-in fade-in \
      ...
      >
        ...
        <div className="p-6">
          ...
          <div className="flex gap-3">
            <button
              onClick={onClose}
              className="flex-1 bg-indigo-600 text-white px-6 py-3 rounded-xl font-medium hover:bg-indigo-700 \
                transition-colors"
            >
              Entendido
            </button>
