...
export const Errors = {
  // Authentication
  invalidCredentials: () => new ApiError(ErrorCode.AUTH_INVALID_CREDENTIALS, 'Email o contraseña incorrectos', 401),

  sessionExpired: () =>
    new ApiError(ErrorCode.AUTH_SESSION_EXPIRED, 'Sesión expirada, por favor inicia sesión de nuevo', 401),

  unauthorized: () => new ApiError(ErrorCode.AUTH_UNAUTHORIZED, 'No tienes permiso para realizar esta acción', 403),
  ...
  // Database
  notFound: (resource: string) => new ApiError(ErrorCode.DB_NOT_FOUND, `${resource} no encontrado`, 404),
  ...
  paymentFailed: (reason: string) => new ApiError(ErrorCode.BUSINESS_PAYMENT_FAILED, `Pago fallido: ${reason}`, 400),

  // Server
  internalError: () => new ApiError(ErrorCode.SERVER_INTERNAL_ERROR, 'Error interno del servidor', 500),
