...
export async function GET(req: NextRequest) {
  try {
    ...
    if (busqueda) {
      where.numeroFactura = { contains: busqueda, mode: 'insensitive' };
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const numeroFactura = `F-${year}-${String(numero).padStart(6, '0')}`;
