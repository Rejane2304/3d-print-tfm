        ...
        data: {
          ...
          items: {
            create: items.map((item: CartItemInput) => ({
              ...
              subtotal: new Prisma.Decimal(item.product.price).mul(item.quantity).toNumber(),
              category: item.product.category?.name || 'Uncategorized',
              material: (item.product.material?.toUpperCase() as 'PLA' | 'PETG') || 'PLA',
            })),
          },
        },
        include: {
...
export const POST = withErrorHandler(async (req: NextRequest) => {
  ...
  // 6. Validate coupon
  const { couponId, couponDiscount } = await validateCoupon(couponCode, subtotal);
