 ...
 * Páginas: /admin/orders/[id]/messages
 */
...
describe('Sistema de Mensajería - API Admin', () => {
  ...
  describe('GET /api/admin/messages', () => {
    ...
    it('debe listar mensajes de un pedido', async () => {
      const response = await fetch(`http://localhost:3000/api/admin/messages?pedidoId=${pedidoId}`, {
        headers: {
          'Cookie': 'next-auth.session-token=admin-token'
        }
      });
  ...
  describe('DELETE /api/admin/messages/[id]', () => {
    it('debe retornar 401 si no está autenticado', async () => {
      const response = await fetch('http://localhost:3000/api/admin/messages/test-id', {
        method: 'DELETE',
      });
      expect(response.status).toBe(401);
    });
...
describe('Sistema de Mensajería - Página UI', () => {
  describe('GET /admin/orders/[id]/messages', () => {
    it('debe cargar página de mensajes del pedido', async () => {
      const response = await fetch('http://localhost:3000/admin/orders/test-id/messages', {
        headers: {
          'Cookie': 'next-auth.session-token=admin-token'
        }
      });
