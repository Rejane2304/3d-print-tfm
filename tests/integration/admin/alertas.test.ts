...
describe('Sistema de Alertas - API Admin', () => {
  ...
  describe('GET /api/admin/alertas', () => {
    ...
    it('debe filtrar por tipo de alerta', async () => {
      const response = await fetch('http://localhost:3000/api/admin/alertas?tipo=STOCK_BAJO', {
        headers: {
          'Cookie': 'next-auth.session-token=admin-token'
        }
      });
    ...
    it('debe filtrar por severidad', async () => {
      const response = await fetch('http://localhost:3000/api/admin/alertas?severidad=ALTA', {
        headers: {
          'Cookie': 'next-auth.session-token=admin-token'
        }
      });
    ...
    it('debe filtrar por estado', async () => {
      const response = await fetch('http://localhost:3000/api/admin/alertas?estado=PENDIENTE', {
        headers: {
          'Cookie': 'next-auth.session-token=admin-token'
        }
      });
  ...
  describe('PATCH /api/admin/alertas', () => {
    it('debe retornar 401 si no está autenticado', async () => {
      const response = await fetch('http://localhost:3000/api/admin/alertas', {
        method: 'PATCH',
  ...
  describe('DELETE /api/admin/alertas/[id]', () => {
    it('debe retornar 401 si no está autenticado', async () => {
      const response = await fetch(`http://localhost:3000/api/admin/alertas/${alertaId}`, {
        method: 'DELETE',
      });
      expect(response.status).toBe(401);
    });
...
describe('Sistema de Alertas - Página UI', () => {
  describe('GET /admin/alertas', () => {
    it('debe cargar página de alertas', async () => {
      const response = await fetch('http://localhost:3000/admin/alertas', {
        headers: {
          'Cookie': 'next-auth.session-token=admin-token'
        }
      });
