 ...
 * Páginas: /admin/orders, /admin/orders/[id]
 */
...
describe('Gestión de Pedidos - API Admin', () => {
  ...
  describe('GET /api/admin/orders/[id]', () => {
    it('debe retornar 401 si no está autenticado', async () => {
      const response = await fetch('http://localhost:3000/api/admin/orders/test-id');
      expect(response.status).toBe(401);
...
describe('Gestión de Pedidos - Página UI', () => {
  ...
  describe('GET /admin/orders/[id]', () => {
    it('debe cargar página de detalle de pedido', async () => {
      const response = await fetch('http://localhost:3000/admin/orders/test-id', {
        headers: {
          'Cookie': 'next-auth.session-token=admin-token'
        }
      });
