...
import { withTestTransaction, resetTestDatabase, prisma, getDBInfo } from '../../helpers';
...
describe('Example: Transaction Safety Demo', () => {
  ...
  it(
    'should create data that rolls back',
  ...
  it(
    'should confirm previous test data was rolled back',
    withTestTransaction(async tx => {
  ...
  it(
    'should handle multiple operations in one transaction',
    withTestTransaction(async tx => {
      // Create category
  ...
  it(
    'should allow using global prisma for setup, tx for isolation',
    withTestTransaction(async tx => {
  ...
  it(
    'should demonstrate error handling in transactions',
    withTestTransaction(async tx => {
  ...
  it('should verify no data leaked from previous tests', async () => {
    ...
    await prisma.user.deleteMany({
      where: {
        email: {
          in: ['temp@example.com', 'new@example.com', 'unique@example.com'],
        },
      },
    });

    const tempEmails = ['temp@example.com', 'new@example.com', 'unique@example.com'];
...
describe('Example: Best Practices', () => {
  ...
  it(
    'DEMO: Always use tx inside withTestTransaction',
    withTestTransaction(async tx => {
  ...
  it(
    'DEMO: Handle async operations properly',
    withTestTransaction(async tx => {
  ...
  it(
    'DEMO: Test assertions work normally',
    withTestTransaction(async tx => {
      const user = await tx.user.findFirst({
        where: { email: 'admin@test.com' },
      });
